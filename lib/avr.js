...
module.exports = {
	init: async function() {
		...
		// sanitize cached names
		this.cachedDevices.forEach(device => {
			if (device.zone1) {
				device.zone1.name = sanitizeName(device.zone1.name)
				if (device.zone1.inputs) {
					device.zone1.inputs.forEach(input => {
						input.name = sanitizeName(input.name)
					})
				}
				if (device.zone1.volume) {
					device.zone1.volume.name = sanitizeName(device.zone1.volume.name)
				}
			}
			for (const i of [2, 3, 4]) {
				if (device[`zone${i}`]) {
					device[`zone${i}`].name = sanitizeName(device[`zone${i}`].name)
					if (device[`zone${i}`].inputs) {
						device[`zone${i}`].inputs.forEach(input => {
							input.name = sanitizeName(input.name)
						})
					}
					if (device[`zone${i}`].volume) {
						device[`zone${i}`].volume.name = sanitizeName(device[`zone${i}`].volume.name)
					}
				}
			}
		})
		...
		for (const config of this.receivers) {
			...
			if (deviceConfig) {
				...
				deviceConfig.zone1.name = sanitizeName(config.name || deviceConfig.zone1.name)
	...
	try {
		const newConfig = {
			...
			zone1: {
				...
				volume: {
					name: sanitizeName(`${config.name} Volume`),
		...
		for (const i of [2,3,4]) {
			if (config.features[`Zone_${i}`] && config.features[`Zone_${i}`][0] === '1') {
				...
				newConfig[`zone${i}`] = {
					...
					name: sanitizeName(`${config.name} Zone${i}`),
					...
					volume: {
						name: sanitizeName(`${config.name} Zone${i} Volume`),
						type: config.volumeAccessory
					}
				}
			}
...
const getInputs = function(config) {
	...
	for (const key in config.inputs) {       
		availableInputs.push({
			key: syncKey(key),
			name: sanitizeName(config.inputs[key][0])
		})
	}
